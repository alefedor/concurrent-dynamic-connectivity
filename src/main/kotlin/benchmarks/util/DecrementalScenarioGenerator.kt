...
class DecrementalScenarioGenerator {
    private val incrementalScenarioGenerator = IncrementalScenarioGenerator()
    ...
    fun generate(graph: Graph, threads: Int): Scenario {
        ...
        for (threadQueries in queries)
            for (i in threadQueries.indices)
                threadQueries[i] = bidirectionalEdge(threadQueries[i].from(), threadQueries[i].to()).edgeToQuery(QueryType.REMOVE_EDGE)
